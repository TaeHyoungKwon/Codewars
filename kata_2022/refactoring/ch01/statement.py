...
def statement(invoice: dict[str, Any], plays: dict[str, Any]) -> str:
    ...
    for performance in invoice["performances"]:
        this_amount = get_amount_for(performance, get_play_for(performance, plays))
        ...
        result += f'\t{get_play_for(performance, plays)["name"]}: {dollar_format(this_amount / 100)} ({performance["audience"]} Seats)\n'
